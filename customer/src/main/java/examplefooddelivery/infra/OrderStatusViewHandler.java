...
public class OrderStatusViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenOrderCanceled_then_UPDATE_8(@Payload OrderCanceled orderCanceled) {
        try {
            if (!orderCanceled.validate()) return;
                // view 객체 조회
            Optional<OrderStatus> orderStatusOptional = orderStatusRepository.findById(orderCanceled.getId());

            if( orderStatusOptional.isPresent()) {
                 OrderStatus orderStatus = orderStatusOptional.get();
            // view 객체에 이벤트의 eventDirectValue 를 set 함
                orderStatus.setStatus("주문 취소");    
                // view 레파지 토리에 save
                 orderStatusRepository.save(orderStatus);
                }
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenRejected_then_UPDATE_9(@Payload Rejected rejected) {
        try {
